...
static void DoAnalog3D() {
         ...
         3. ignore a bit-0x4 false positive until it finally clears. */
    {
        enum { BIT4_SUSPECT_FRAMES = 15, CUTSCENE_RELEASE_FRAMES = 12,
               DOOR_RELEASE_FRAMES = 3 };
        static int s_bit4Frames = 0;
        static int s_releaseFrames = 0;
        static bool s_cutsceneLatched = false;
        static bool s_letterboxSeenDuringLatch = false; /* true if lb>0 at any point during this latch */
        ...
        /* Latch on bit4-confirmed scripted sequences or the r1PreCut pre-cut heuristic.
           hardCut (STATUS 0x40000000 / 0x02000000) is intentionally NOT a latch trigger:
           it fires during door transitions as well as real cutscenes, so latching on it
           caused a 12-frame release tail on every door — the regression vs. the original
           release which never checked those bits at all.  Real cutscenes are covered by
           letterbox (primary) and bit4/r1PreCut (secondary).  hardCut still contributes
           to blockCutscene directly so it suppresses analog for its own duration only,
           with no countdown tail. */
        if (bit4ConfirmsScript || r1PreCut) {
            if (!s_cutsceneLatched) s_letterboxSeenDuringLatch = false; /* fresh latch */
            s_cutsceneLatched = true;
            s_releaseFrames = 0;
        } else if (s_cutsceneLatched) {
            if (letterbox) s_letterboxSeenDuringLatch = true;
            /* Door transitions never show letterbox — use a short countdown.
               Real cutscenes raise letterbox — use the full countdown to avoid
               releasing too early between cutscene segments. */
            int releaseThreshold = s_letterboxSeenDuringLatch
                ? CUTSCENE_RELEASE_FRAMES : DOOR_RELEASE_FRAMES;
            if (stableGameplay) {
                if (s_releaseFrames < releaseThreshold) s_releaseFrames++;
                if (s_releaseFrames >= releaseThreshold) {
                    s_cutsceneLatched = false;
                    s_releaseFrames = 0;
                    s_letterboxSeenDuringLatch = false;
                }
            } else {
                s_releaseFrames = 0;
            }
